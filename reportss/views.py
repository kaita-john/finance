        ...
        try:
            ...
            if accounttype and accounttype != "" and accounttype != "null":
                if not bankaccount or bankaccount == "" or bankaccount == "null":
                    return Response({'detail': f"Bank Account is required"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            receipt_dates = [date for date in receipt_dates if date is not None]
            pik_dates = [date for date in pik_dates if date is not None]
            grant_dates = [date for date in grant_dates if date is not None]
            bursary_dates = [date for date in bursary_dates if date is not None]
...
class LedgerView(SchoolIdMixin,  DefaultMixin, generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        bursaryQuerySet = Bursary.objects.filter(
            posted=True,
            school_id=school_id,
        ...
        bursaryQuerySet = bursaryQuerySet if bursaryQuerySet.exists() else Bursary.objects.none()

        print(f"{len(collectionQuerySet)}")
        print(f"{len(pikQuerySet)}")
        print(f"{len(voucherQuerySet)}")
        print(f"{len(grantsQuerySet)}")
        print(f"{len(bursaryQuerySet)}")
